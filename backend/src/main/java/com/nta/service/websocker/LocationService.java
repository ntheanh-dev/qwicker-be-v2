...
public class LocationService {
    ...
    public Set<String> getShippersByGeoHash(final double latitude, final double longitude) {
        final String geoHash = geoHashService.getGeohash(latitude, longitude);
        return getShippersByGeoHash(geoHash);
    }

    public Set<String> getShippersByGeoHash(final String geoHash) {
        final Map<String, Object> result = redisService.hashGetAll(geoHash);
    ...
    public void updateLocation(
        ...
        if(onlineOfflineService.isUserOnline(userDetail.getId())) {
            final String newGeoHash = geoHashService.getGeohash(locationMessage.getLatitude(), locationMessage.getLongitude());
            //--------Delete Old Location if it present ----------------
            if (locationMessage.getPrevLatitude() != 0.0 && locationMessage.getLongitude() != 0.0) {
                final String oldGeoHash = geoHashService.getGeohash(locationMessage.getPrevLatitude(), locationMessage.getPrevLongitude());
                redisService.delete(oldGeoHash, locationMessage.getShipperId());
            }
            final var vehicle = shipperService.getVehicleByUserId(userDetail.getId()).orElseThrow(
                    () -> new AppException(ErrorCode.VEHICLE_NOT_FOUND));
            //--------Update Location-------------------
            if(redisService.isRedisLive()) {
                redisService.hashSet(newGeoHash, locationMessage.getShipperId(),
                        ShipperDetailCache
                                .builder()
